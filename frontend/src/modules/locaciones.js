...
function renderLocaciones() {
  ...
  const subtabs = `<div class="loc-subtabs">
    <button class="loc-subtab ${st.sub === 'repo' ? 'on' : ''}" onclick="locSetSub('repo')">Repositorio</button>
    <button class="loc-subtab ${st.sub === 'scout' ? 'on' : ''}" onclick="locSetSub('scout')">Plan de Scouting</button>
  </div>`;
...
function locRepoHTML(project) {
  ...
  const filtros = `<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
    <div class="loc-filters">${['todas', 'confirmada', 'candidata', 'descartada'].map(f => `<button class="loc-chip ${st.filtro === f ? 'on' : ''}" onclick="locSetFiltro('${f}')">${f === 'todas' ? 'Todas' : window.LOC_ESTADOS[f]}</button>`).join('')}</div>
    <button class="btn btn-primary btn-sm" onclick="openLocAdd()">+ Agregar locación</button>
  </div>`;
  let grid;
  if (!list.length) {
...
function locCardHTML(uso) {
  ...
  return `<div class="loc-card" onclick="openLocDetail('${l.locId}')">
...
function openLocDetail(locId) {
  ...
  const contactosHTML = (l.contactos || []).map((c, i) => `
            <div style="border:1px solid var(--border);border-radius:8px;padding:9px 10px;margin-bottom:8px;">
              <div style="display:flex;gap:8px;align-items:flex-end;">
                <span style="flex:1;"><label style="font-size:11px;color:var(--ink-mut);">Nombre</label><input class="input" value="${e(c.nombre || '')}" onchange="locSetContacto('${locId}',${i},'nombre',this.value)"></span>
                <button class="btn btn-secondary btn-sm" style="color:#d08;border-color:rgba(210,0,80,.4);" onclick="locDelContacto('${locId}',${i})" title="Quitar contacto">×</button>
              </div>
              <div style="margin-top:7px;"><label style="font-size:11px;color:var(--ink-mut);">Relación con la locación</label><input class="input" value="${e(c.relacion || '')}" placeholder="Dueño · Gerente · Administrador · Encargado · Hijo del dueño · Recepcionista…" onchange="locSetContacto('${locId}',${i},'relacion',this.value)"></div>
              <div style="display:flex;gap:8px;margin-top:7px;"><span style="flex:1;"><label style="font-size:11px;color:var(--ink-mut);">Mail</label><input class="input" value="${e(c.mail || '')}" onchange="locSetContacto('${locId}',${i},'mail',this.value)"></span><span style="flex:1;"><label style="font-size:11px;color:var(--ink-mut);">Teléfono</label><input class="input" value="${e(c.tel || '')}" onchange="locSetContacto('${locId}',${i},'tel',this.value)"></span></div>
              <div style="margin-top:7px;"><label style="font-size:11px;color:var(--ink-mut);">Observaciones</label><input class="input" value="${e(c.obs || '')}" onchange="locSetContacto('${locId}',${i},'obs',this.value)"></div>
  ...
  const galeria = (l.fotos || []).map((f, i) => `<div class="loc-thumb" draggable="true"
      ...
      <img id="lf_${locId}_${i}" src="${safeUrl(f.url || f._signedUrl || _LOC_FOTO_PLACEHOLDER)}" onclick="locLightbox('${locId}',${i})" style="pointer-events:none;">
      <button class="loc-thumb-x" onclick="event.stopPropagation();locDelFoto('${locId}',${i})" title="Quitar">×</button>
  ...
  const estadoChips = uso ? Object.keys(window.LOC_ESTADOS).map(s => `<button class="loc-chip ${uso.estado === s ? 'on' : ''}" onclick="locSetEstado('${locId}','${s}')">${window.LOC_ESTADOS[s]}</button>`).join('') : '';
  document.getElementById('modalRoot').innerHTML = `<div class="modal-backdrop"><div class="modal" onclick="event.stopPropagation()" style="max-width:760px;width:94vw;max-height:88vh;overflow:auto;">
    <div class="modal-header"><div class="modal-title">${e(l.nombre || 'Locación')} · ${e(l.locId)}</div><button class="go-x" onclick="window.closeModal()" style="background:none;border:none;font-size:22px;cursor:pointer;color:var(--ink-mut);">×</button></div>
      ...
      <div class="loc-modal-grid">
        <div>
          <div class="loc-block">
            ...
            <div class="loc-field"><label>Nombre</label><input class="input" value="${e(l.nombre || '')}" onchange="locSetBD('${locId}','nombre',this.value)"></div>
            <div class="loc-field"><label>Dirección</label><input class="input" value="${e(l.direccion || '')}" placeholder="Calle 123" onchange="locSetBD('${locId}','direccion',this.value)"></div>
            <div class="loc-field"><label>Segunda línea (opcional)</label><input class="input" value="${e(l.direccion2 || '')}" placeholder="Torre B · Depto 302 · Casa 7 · Villa…" onchange="locSetBD('${locId}','direccion2',this.value)"></div>
            <div class="loc-field" style="display:flex;gap:8px;"><span style="flex:1;"><label>Comuna</label><input class="input" value="${e(l.comuna || '')}" onchange="locSetBD('${locId}','comuna',this.value)"></span><span style="flex:1;"><label>Ciudad</label><input class="input" value="${e(l.ciudad || '')}" onchange="locSetBD('${locId}','ciudad',this.value)"></span></div>
            <div class="loc-field"><label>Región</label>${regionSelectHTML(l.region, {onchange:`locSetBD('${locId}','region',this.value)`})}</div>
            <div class="loc-field"><label>Link Google Maps</label><input class="input" value="${e(l.maps || '')}" placeholder="https://maps.google.com/…" onchange="locSetBD('${locId}','maps',this.value)"></div>
            <div class="loc-field"><label>Notas generales</label><textarea class="input" rows="2" onchange="locSetBD('${locId}','notas',this.value)">${e(l.notas || '')}</textarea></div>
          </div>
          <div class="loc-block">
            <div class="loc-block-h">Contactos de la locación <span class="loc-block-tag bd">BD · reutilizable</span></div>
            ${contactosHTML}
            <button class="btn btn-secondary btn-sm" onclick="locAddContacto('${locId}')">+ Agregar contacto</button>
          ...
          <div class="loc-block">
            ...
            <div class="loc-field"><label>Orientación</label><select class="input" onchange="locSetBD('${locId}','orientacion',this.value)">${orientOpts}</select></div>
          </div>
        </div>
        <div>
          <div class="loc-block">
            <div class="loc-block-h">Galería de scouting <span class="loc-block-tag bd">BD · reutilizable</span></div>
            <div class="loc-gallery">${galeria}<label class="loc-foto-add" title="Agregar fotos (se comprimen solas)">+<input type="file" accept="image/*" multiple style="display:none" onchange="locAddFotos('${locId}',this)"></label></div>
            <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:8px;flex-wrap:wrap;">
              ...
              ${(l.fotos || []).length ? `<button class="btn btn-secondary btn-sm" onclick="locDescargarFotos('${locId}')">⬇ Descargar todo</button>` : ''}
            </div>
          </div>
          ${uso ? `<div class="loc-block">
            <div class="loc-block-h">Uso en este proyecto <span class="loc-block-tag proj">solo de este proyecto</span></div>
            <div class="loc-field"><label>Costo negociado (CLP)</label><input class="input" inputmode="numeric" value="${uso.costo || 0}" onchange="locSetProj('${locId}','costo',this.value)"></div>
            <div class="loc-field"><label>Estado de contratación</label><input class="input" value="${e(uso.contratacion || '')}" placeholder="Contrato firmado / En conversación / Propia…" onchange="locSetProj('${locId}','contratacion',this.value)"></div>
            <div class="loc-field"><label>Notas del proyecto</label><textarea class="input" rows="2" onchange="locSetProj('${locId}','notasProy',this.value)">${e(uso.notasProy || '')}</textarea></div>
          </div>` : ''}
        </div>
      </div>
      ${uso ? `<div style="margin-top:4px;"><button class="btn btn-secondary btn-sm" style="color:#d08;border-color:rgba(210,0,80,.4);" onclick="locRemoveFromProject('${locId}')">Quitar de este proyecto</button> <span style="font-size:11px;color:var(--ink-faint);">No se borra de la Base de Datos; solo deja de usarse en este proyecto.</span></div>` : ''}
    </div>
    <div class="modal-footer">${_bdPuedeArchivar() ? `<button class="btn btn-ghost btn-sm" style="color:var(--accent-deep);margin-right:auto;" onclick="window.archivarLocacionModal('${locId}')">Archivar</button>` : ''}<button class="btn btn-primary" onclick="window.closeModal()">Listo</button></div>
  </div></div>`;
}
...
function _locLbRender() {
  ...
  div.innerHTML = `<button class="loc-lightbox-x" onclick="_locLbClose()">×</button>`
    + (multi ? `<button class="loc-lb-nav prev" onclick="event.stopPropagation();_locLbMove(-1)" style="position:fixed;left:18px;top:50%;transform:translateY(-50%);font-size:34px;background:rgba(0,0,0,.5);color:#fff;border:none;border-radius:50%;width:52px;height:52px;cursor:pointer;z-index:2;">‹</button>` : '')
    + `<img id="locLbImg" src="${safeUrl(_locLbSrc(_LOC_LB.locId, _LOC_LB.i))}" alt="">`
    + (multi ? `<button class="loc-lb-nav next" onclick="event.stopPropagation();_locLbMove(1)" style="position:fixed;right:18px;top:50%;transform:translateY(-50%);font-size:34px;background:rgba(0,0,0,.5);color:#fff;border:none;border-radius:50%;width:52px;height:52px;cursor:pointer;z-index:2;">›</button>` : '')
...
function openLocAdd() {
  ...
  const e = escapeHtml;
  document.getElementById('modalRoot').innerHTML = `<div class="modal-backdrop" onclick="window.closeModal()"><div class="modal" onclick="event.stopPropagation()" style="max-width:520px;">
    <div class="modal-header"><div class="modal-title">Agregar locación</div><button class="go-x" onclick="window.closeModal()" style="background:none;border:none;font-size:22px;cursor:pointer;color:var(--ink-mut);">×</button></div>
    <div class="modal-body">
      <div class="loc-filters" style="margin:0 0 12px;"><button class="loc-chip on" id="lam_new" onclick="locAddMode('new')">Crear nueva</button><button class="loc-chip" id="lam_bd" onclick="locAddMode('bd')">Reutilizar de la BD</button></div>
      <div id="locAddNew">
    ...
    <div class="modal-footer"><button class="btn btn-secondary" onclick="window.closeModal()">Cancelar</button><button class="btn btn-primary" onclick="saveLocAdd()">Agregar</button></div>
  </div></div>`;
...
function comboboxFilterLocScout(inputEl) {
  ...
  if (matched.length === 0) {
    ...
    dropdown.innerHTML = matched.slice(0, 20).map(l => `<div class="combobox-option" onmousedown="comboboxSelect(this, '${escapeHtml(l.nombre || '')}')"><div class="combobox-option-main">${escapeHtml(l.nombre || '(sin nombre)')}</div>${l.direccion ? `<div class="combobox-option-meta">${escapeHtml(l.direccion)}</div>` : ''}</div>`).join('') + (matched.length > 20 ? `<div class="combobox-more">+ ${matched.length - 20} más — sigue tipeando</div>` : '');
  }
...
function locScoutingHTML(project) {
  ...
  const quienes = s.quienes.map((q, i) => `<span class="scout-chip"><span class="combobox-wrap person-combobox cbx-anchored" style="min-width:150px;"><input class="combobox-input" value="${e(q)}" placeholder="Buscar persona…" autocomplete="off" onfocus="comboboxOpen(this)" oninput="window.comboboxFilter(this)" onblur="comboboxCloseDelayed(this)" onchange="locScoutSetQuien(${i}, this.value)"><div class="combobox-dropdown" hidden></div></span><button class="x" onclick="locScoutDelQuien(${i})" title="Quitar">×</button></span>`).join('');
  const nFilas = s.filas.length;
  let _pIdx = -1;
  const filasHTML = s.filas.map((f, i) => {
    const ln = (i === nFilas - 1) ? '' : '<span class="ln"></span>';
    if (f.tipo === 'traslado') {
      return `<div class="scout-trasl"><div></div><div class="scout-rail">${ln}</div><div class="scout-seg">⤷ Traslado <input class="scout-dur-in" value="${e(f.dur || '')}" title="HHMM: 100 = 1 hora · tiempo de viaje" onchange="locScoutSetFila(${i},'dur',this.value,true)"> <span style="font-size:11px;color:var(--ink-faint);">de viaje</span></div></div>`;
    }
    _pIdx++; const pIdx = _pIdx;
    const clock = times[i] && times[i].inicio != null ? prFmtClock(times[i].inicio) : '—';
    const horaCell = (i === 0)
      ? `<input class="scout-time-in" value="${e(s.inicio || '')}" inputmode="numeric" maxlength="5" title="Hora de inicio del plan (editable)" onchange="locScoutSet('inicio', this.value)">`
      : `<span>${clock}</span>`;
    ...
    const dotNoBD = (f.resp && !_bdP) ? `<button type="button" title="No está en la Base de Datos · clic para agregarla" onclick="locScoutAddContactoBD('${encodeURIComponent(f.resp)}')" style="border:none;background:none;cursor:pointer;color:var(--warning);font-weight:700;font-size:13px;padding:0 3px;">●</button>` : '';
    return `<div class="scout-node" ondragover="locScoutDragOver(event,${pIdx})" ondragleave="locScoutDragLeave(event)" ondrop="locScoutDrop(event,${pIdx})">
      ...
      <div class="scout-card">
        <div class="scout-p-top">
          ...
          <span class="combobox-wrap person-combobox" style="flex:1;min-width:0;"><input class="input combobox-input" value="${e(_nomParada)}" placeholder="Locación o texto libre…" autocomplete="off" style="font-weight:600;width:100%;" onfocus="comboboxFilterLocScout(this)" oninput="comboboxFilterLocScout(this)" onblur="comboboxCloseDelayed(this)" onchange="locScoutSetParadaLoc(${i}, this.value)"><div class="combobox-dropdown" hidden></div></span>
          ${(esLibre && _nomParada) ? `<button type="button" title="No está en la BD de locaciones · clic para agregarla" onclick="locScoutAddLocBD(${pIdx})" style="border:none;background:none;cursor:pointer;color:var(--warning);font-weight:700;font-size:14px;padding:0 2px;flex:0 0 auto;">●</button>` : ''}
          <span class="scout-badge ${esLibre ? 'libre' : 'loc'}">${esLibre ? 'Parada libre' : 'Locación'}</span>
          <button class="scout-del" onclick="locScoutDelParada(${i})" title="Quitar parada (y su traslado)">×</button>
        </div>
        <div class="scout-p-row">${dirTexto}${verMaps}<button class="btn btn-secondary btn-sm" onclick="locScoutBuscarMaps(${i})" title="Abre Google Maps con este texto para que verifiques la ubicación; luego pega el link aquí">Buscar en Maps</button><input class="input" style="flex:1;min-width:150px;" value="${e(f.maps || '')}" placeholder="Pega aquí el link de Google Maps…" onchange="locScoutSetParadaMaps(${i}, this.value)"></div>
        <div class="scout-p-row"><span class="combobox-wrap person-combobox" style="min-width:150px;"><input class="input combobox-input" value="${e(respName)}" placeholder="Contacto…" autocomplete="off" onfocus="comboboxOpen(this)" oninput="window.comboboxFilter(this)" onblur="comboboxCloseDelayed(this)" onchange="locScoutSetFila(${i},'resp',this.value,true)"><div class="combobox-dropdown" hidden></div></span>${dotNoBD}<input class="input" style="width:130px;" value="${e(respTel)}" placeholder="Celular" onchange="locScoutSetFila(${i},'respTel',this.value)"><input class="input" style="flex:1;min-width:160px;" value="${e(f.revisar || '')}" placeholder="Notas (ej.: tocar el timbre dos veces)" onchange="locScoutSetFila(${i},'revisar',this.value)"></div>
      </div>
    </div>`;
  }).join('');
  return `<div class="cot-card" style="padding:16px;margin-bottom:14px;">
    <div style="display:flex;gap:16px;flex-wrap:wrap;align-items:flex-end;">
      <div class="loc-field" style="margin:0;"><label>Fecha de scouting</label><input type="date" class="input" value="${e(s.fecha || '')}" onchange="locScoutSet('fecha',this.value)"></div>
      <div class="loc-field" style="margin:0;"><label>Hora de inicio</label><input class="input" style="width:90px;" inputmode="numeric" maxlength="5" value="${e(s.inicio || '')}" placeholder="09:00" onchange="locScoutSet('inicio',this.value)"></div>
    </div>
    <div class="scout-route">
      ...
      <button class="btn btn-primary btn-sm" onclick="locScoutRutaEntera()">Crear ruta en Maps ↗</button>
    </div>
  </div>
  <div class="cot-card" style="padding:14px 16px;margin-bottom:14px;">
    <div class="loc-field" style="margin:0;"><label>Quiénes van</label>
      <div class="scout-chips" style="margin-top:6px;">${quienes}<button class="scout-chip-add" onclick="locScoutAddQuien()">+ Persona</button></div>
    </div>
  </div>
  <div class="cot-card" style="padding:14px 16px;">
    ...
    <div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
      <button class="btn btn-secondary btn-sm" onclick="locScoutAddParada()">+ Parada</button>
      <span style="flex:1;"></span>
      <button class="btn btn-primary btn-sm" onclick="scoutPreviewPDF()">Exportar PDF</button>
    </div>
  </div>`;
}
...
function scoutPreviewPDF() {
  ...
  document.getElementById('modalRoot').innerHTML = `<div class="modal-backdrop"><div class="modal" onclick="event.stopPropagation()" style="max-width:1000px;width:96vw;padding:0;overflow:hidden;">
    ...
    <div style="display:flex;flex-direction:column;min-height:60vh;max-height:74vh;">
      <div style="flex:1;display:flex;flex-direction:column;min-width:0;background:#2a2a27;">
        <div style="display:flex;align-items:center;gap:8px;padding:8px 14px;background:var(--bg-surface,#222);border-bottom:1px solid var(--rule,#34342f);flex-wrap:wrap;">
          <button class="btn btn-sm" onclick="window.CotPreview.setZoom(window.CotPreview.zoom-10)" title="Alejar">−</button>
          ...
          <button class="btn btn-sm" onclick="window.CotPreview.setZoom(window.CotPreview.zoom+10)" title="Acercar">+</button>
          <span style="width:1px;height:18px;background:var(--rule,#34342f);margin:0 3px;"></span>
          <button class="btn btn-sm" id="cotPrevFitPage" onclick="window.CotPreview.setMode('page')">Ajustar</button>
          <button class="btn btn-sm" id="cotPrevFitWidth" onclick="window.CotPreview.setMode('width')">Ancho</button>
    ...
    <div class="modal-footer" style="padding:12px 18px;justify-content:flex-end;gap:8px;"><button class="btn" onclick="window.closeModal()">Cerrar</button><button class="btn btn-primary" onclick="scoutPreviewGenerar()">Exportar PDF</button></div>
  </div></div>`;
