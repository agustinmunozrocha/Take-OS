...
- **Cambios de base de datos: primero al repo, después a producción.** Todo cambio de BD es un **archivo de migración** en `supabase/migrations/`; se aplica a producción con **`supabase db push`** (el repo está linkeado al proyecto de prod) y el archivo queda commiteado en `main`. **Nunca** se aplica un cambio de esquema o datos directo a producción por el conector MCP ni por el editor SQL: eso desincroniza la base respecto del código (doctrina «BD en código», ADR-023 / Roadmap §5.1). El conector MCP de Supabase es solo para **inspección de lectura** y pruebas en transacción revertida (`BEGIN … ROLLBACK`).
