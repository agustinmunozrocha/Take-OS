...
**Estado:** **Aprobada y ejecutada** — Prioridad #1, #2 **y #3 CERRADAS**: la modularización con Vite está **en producción** desde el **corte del 24-jul-2026**. Rama única `main`; el monolito quedó dormido solo como rollback.
...
**Documentos relacionados:** PRD V3.8 · ADR de Backend v1.14 · Roadmap Operativo v1.12 · Seguridad OWASP Top 10:2025 v1.7

> **✅ El eje "producción ≠ staging" TERMINÓ el 24-jul-2026.** El corte a producción se ejecutó: **producción corre la arquitectura modular** que describe este documento (build de Vite publicada por GitHub Pages en modo `workflow`), en la **rama única `main`**. La divergencia de 189 commits desapareció (las ramas de etapa se mergearon y borraron; `Cambios_post_modularizacion.md` quedó saldado). El monolito `index.html` sigue en el árbol **solo como rollback** (Pages → `legacy`) y se eliminará tras el período de estabilidad. Lo que este documento describe del cliente modular es, desde el corte, **el estado de producción**.

> **Cambios respecto a v1.9** (esta versión — consolida el **hito del 24-jul-2026**):
> 1. **✅ Corte a producción HECHO (§2.4, §5, §7).** Merge de la rama modular a `main` vía PR con ensayo (preview branch de Supabase corrió las 9 migraciones pendientes sobre copia del esquema de prod) → Branching las aplicó → flip de GitHub Pages de `legacy` a **`workflow`** (publica `frontend/dist` vía `deploy.yml`) → verificación en vivo (smoke tests 10/10). El "404 real" no se manifestó. **Rollback:** Pages a `legacy` (el monolito dormido revive).
> 2. **✅ Gate B CERRADO (§2.5, §6):** 0 políticas `mvp_`; RLS real por organización y rol en 85/85 tablas; **aislamiento entre organizaciones verificado** (tests de cruce en verde); guard server-side del soft-delete de `projects`; REVOKE TRUNCATE/TRIGGER/REFERENCES a `anon`/`authenticated`; políticas de Storage capturadas en migración (+ fix `COALESCE`); test de controles `supabase/tests/gateB_controles_seguridad.sql` (5 controles, verde en prod — correr tras cada deploy).
> 3. **Cifras únicas (§2.2, pie):** producción **85 tablas (85 RLS) · 173 policies · 82 funciones · 28 migraciones**; staging con el **mismo set** (R4 cumplida). Desaparecen las "cifras duales" y las "~5 migraciones sin nombrar".
> 4. **CSP estricta EN PRODUCCIÓN** (`script-src` sin `unsafe-inline`; queda `style-src`, proyecto S).
> 5. **Textos legales entregados** por los abogados (24-jul; integración en la app pendiente) · **dominio `rizoraapp.com` registrado** (landing en Squarespace; la app sigue en Pages). Los remanentes de A01 (`contacts` vs externos; snapshots por org) quedan en Gate C.
> 6. Build de producción: **V11.41.0** (paridad V11.40 portada + fixes del corte).

> **Cambios respecto a v1.8** (renombre a **Rizora** + salida del CTO, por orden de Agustín del 10-jul):
...
- Se **capturó la base como migración base** y se verificó **reproducible** (se reconstruye desde cero con `db reset`). **Conteo único (v1.10): 28 migraciones, con el mismo set en producción y staging** (R4 cumplida el 24-jul-2026; el conteo dual "9/14/~5 sin nombrar" quedó superado — la fuente de verdad del listado es `supabase/migrations/`). El arco: esquema completo, triggers, cron, revocaciones (`anon`/`service_role`), provisión autocontenida, endurecimiento, concurrencia y persistencias por módulo, las 6 renumeradas del corte (`20260724110001..110006`) y las 3 de **Gate B** (`20260724120000/121000/122000` — ver ADR-023/ADR-024).
...
### 2.4. El frontend: la arquitectura modular EN PRODUCCIÓN (el monolito, jubilado)

> **✅ Actualización v1.10 — un solo frontend.** Desde el **corte del 24-jul-2026, producción corre la arquitectura modular** (40 archivos ES Modules en `frontend/src/`, build de Vite publicada por GitHub Pages en modo `workflow`; detalle en §3 y §7). El monolito que describe el bloque de abajo quedó **dormido en el árbol de `main` solo como rollback** (Pages → `legacy`) y se eliminará tras el período de estabilidad. El bloque se conserva como **registro histórico** de lo que fue producción y del porqué del refactor.

**Lo que fue producción — el monolito (registro histórico).** El `index.html` que estuvo en producción era **un solo archivo de ~1,9 MB y ~28.800 líneas** (549 handlers inline). Adentro:
...
**Sobre el CSP — el logro está EN PRODUCCIÓN (v1.10).** La delegación de eventos retiró todos los `onclick` inline y permitió endurecer la CSP a `script-src 'self' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com` — **sin `unsafe-inline`**: el navegador rechaza todo JS inline, propio o inyectado. Era el "premio de seguridad" del refactor (OWASP A05) y, con el corte del 24-jul, **rige en el sitio real**. Queda `style-src` con `unsafe-inline` (deuda dimensionada, "proyecto S").
...
| **Gate B** — "Permisos reales para Primate" | ✅ **CERRADO (24 jul 2026)** | Nada. RLS real por organización y rol en las **85/85 tablas** (0 `mvp_`; políticas `b_*` sobre `auth_nivel`/`auth_ve_proyecto`); **aislamiento multi-tenant VERIFICADO** con tests de cruce (en verde) y vigilado por `supabase/tests/gateB_controles_seguridad.sql`; guard server-side del soft-delete; motor de organización activa (`_setOrgActiva`) operativo. |
| **Gate C** — "Listo para datos de terceros" | 🟡 **ACOTADO** | Con los **textos legales ya entregados por los abogados (24-jul-2026)**, queda: **integrarlos en la app** (+ QA de los cinco flujos), el header `frame-ancestors`, los remanentes de A01 (`contacts` vs externos; snapshots por org), la cadena de suministro A03 y la alerta mínima A09. El pentest externo sigue desbloqueado. Deadline Ley 21.719 (1-dic-2026): holgado. |
...
| | **Producción** | **Staging** |
|---|---|---|
| Repo GitHub | `agustinmunozrocha/Take-OS` (rama única `main`) | `agustinmunozrocha/takeos-staging` (espejo desplegable del mismo proyecto Vite) |
| Carpeta local (Mac) | `/Users/agustinmunozrocha/Software` | `/Users/agustinmunozrocha/Software-staging` |
| Sitio publicado (GitHub Pages, modo `workflow`) | `https://agustinmunozrocha.github.io/Take-OS/` *(dominio propio `rizoraapp.com` registrado — hoy con landing; conectarlo a la app es pendiente)* | `https://agustinmunozrocha.github.io/takeos-staging/` |
| Base de datos (Supabase) | `zplcgetquwxybkrpmcvl` (REAL) | branch `staging` → `jovroabtwysliryppthh` (PRUEBAS · mismo set de migraciones, R4) |

**La diferencia entre los dos entornos ya no son "dos líneas editadas a mano": es el modo de build de Vite.** El mismo código elige la base por entorno vía `import.meta.env` (`.env.production` / `.env.staging`): `npm run build` → base de producción; `npm run build:staging` → base de staging (el workflow `deploy.yml` decide según el repo). Son **claves publicables**, públicas por diseño y seguras de exponer (el acceso lo protege el RLS):

- **Producción** — `VITE_SUPABASE_URL = 'https://zplcgetquwxybkrpmcvl.supabase.co'` (clave publicable en `.env.production`)
- **Staging** — `VITE_SUPABASE_URL = 'https://jovroabtwysliryppthh.supabase.co'` (clave publicable en `.env.staging`)

### 5.2. Cómo se trabaja y cómo accede otra persona

- **Hacer un cambio:** se trabaja en `frontend/src/` sobre la **rama única `main`** de `/Software` (rama de feature → PR → merge; ver §2.2 para BD). Al mergear, `deploy.yml` reconstruye y publica producción en ~1–2 minutos (verificar con refresh duro, Cmd+Shift+R). Para probar contra la base de staging: `npm run dev` (usa `.env.staging`) o push al repo `takeos-staging`.
- **✅ Sincronizar producción ↔ staging — RESUELTO (24-jul-2026).** El riesgo principal de v1.8 (dos remotos con 189 commits de deriva) quedó cerrado con el corte: **un solo software, una sola rama**. El frontend se construye con **Vite** (`vite build` → `frontend/dist/`, `base: './'`, credenciales por `import.meta.env`) y la BD se aplica por **Branching al mergear** (merge = deploy, §2.2). El "404 real" no se manifestó (deploy verificado en vivo).
...
> **Hallazgos del Informe Técnico (v1.8) — estado al 24-jul-2026 (detalle en el hub OWASP):**
> 1. **✅ CERRADO — el borrado blando ya no elude el permiso.** Se cerró **server-side** (mejor que el fix propuesto de "que kanban llame la RPC"): trigger guard en `projects` (migración `20260724120000`) que exige `eliminar_proyecto='E'` cuando el UPDATE toca `deleted_at`. Un Ejecutivo fabricando la request recibe excepción; el flujo del Admin sigue intacto. Ya no depende de qué camino tome el cliente.
> 2. **⚠ SIGUE ABIERTO — "el externo no lee `contacts`" sigue siendo convención.** Ninguna policy consulta `memberships.tipo`; un externo con un perfil con `bd∈{E,L}` leería la tabla de contactos completa. **Fix pendiente:** policy que restrinja `contacts` para `memberships.tipo='externo'`. Es el remanente principal de A01 (Gate C).
>
> Además: los **snapshots/airbag por organización** quedan **por verificar** (A01-adyacente). Y la **cadena de suministro** (A03) sigue pendiente: sin SRI en los CDN, `supabase-js@2` con major flotante, `xlsx` cargado dos veces — re-auditar sobre el build de Vite. **Nuevo (24-jul):** REVOKE de TRUNCATE/TRIGGER/REFERENCES a `anon`/`authenticated` (TRUNCATE no pasa por RLS) + test de controles `supabase/tests/gateB_controles_seguridad.sql` (correr tras cada deploy).
...
### Prioridad #3 — Modularización del código del frontend · ✅ COMPLETA Y EN PRODUCCIÓN (corte ejecutado el 24-jul-2026)
...
> **La magnitud real (actualizada v1.10).** Esta nota decía "el cimiento, no la casa: 12 de ~1.290 funciones, <1%, 88% en la Etapa 2"; luego "la casa está construida en staging, el corte es el pendiente". **Ambas quedaron atrás: la casa está construida Y HABITADA** — 40 archivos y ~25.300 líneas modularizadas (14 en `lib/` + 25 módulos de negocio), biyección de acciones verificada, **en producción desde el 24-jul-2026** con paridad funcional V11.40 verificada (registro `Cambios_post_modularizacion.md` saldado).
...
> **El objetivo final de seguridad — ✅ LOGRADO Y EN PRODUCCIÓN (v1.10).** El premio era **quitar `'unsafe-inline'` del CSP**, posible solo cuando no queden `onclick`/`<script>` inline. La **delegación de eventos** lo consiguió y el corte del 24-jul lo llevó al sitio real: el `script-src` de producción va **sin `'unsafe-inline'`** (queda `style-src`, deuda "proyecto S"). Cruza con OWASP A05.
...
- **Los gates: A y B cerrados; Gate C quedó acotado.** Gate A cerrado (8-jun); **Gate B cerrado (24-jul: RLS real por org/rol, aislamiento verificado con tests de cruce)**; Gate C —el crítico, antes del beta— quedó corto: con los **textos legales ya entregados por los abogados (24-jul)**, resta integrarlos en la app (+ QA), `frame-ancestors`, los remanentes de A01 y la cadena de suministro. No operemos bajo la idea de que "ya está todo listo", pero lo estructural está cerrado.
...
1. **Prioridad #3 — modularización del frontend: ✅ CERRADA (24-jul-2026).** Todas las etapas hechas (andamiaje, cimiento, módulos de negocio, desacople D0–D4 + FASE HF) **y el corte a producción ejecutado**. Lo que sigue ya no es la modularización: es el **backlog de producto** (Notas de Software WIP), la **integración de los textos legales** y los remanentes de Gate C — más la higiene final de **eliminar el monolito dormido** cuando pase el período de estabilidad.
...
*Documento canónico v1.10 — aprobado y ejecutado. **Cifras vivas únicas** (el corte a producción se hizo el 24-jul-2026; entorno único en la rama `main`):*
- ***Producción**: base Supabase `zplcgetquwxybkrpmcvl` con **85 tablas (85 con RLS), 173 políticas (142 `b_*`, 0 `mvp_`), 82 funciones, 28 migraciones**, reproducible, actualizada por Branching al mergear; staging (`jovroabtwysliryppthh`) con el **mismo set de migraciones** (R4). Frontend en producción: **modular — 40 archivos, ~25.300 líneas** (14 `lib/` + 25 `modules/`), delegación de eventos, ganchos, época, **CSP sin `'unsafe-inline'` en `script-src`**, compuertas `npm run gate`, build **V11.41.0** publicada por el workflow de Pages. El monolito `index.html` queda dormido en el árbol solo como rollback.*

*Las "diferencias abiertas" de v1.8 (77→72 tablas; ~5 migraciones sin nombrar) quedaron explicadas por el drift de censos y superadas con el entorno único. **El corte a producción — el gran pendiente de v1.8/v1.9 — se ejecutó el 24 de julio de 2026.** Consolidado ese mismo día.*
